%%
%% This is file `ufrj-quickref.tex',
%% generated with the docstrip utility.
%%
%% The original source files were:
%%
%% ufrj.dtx  (with options: `quickref')
%% 
...
%%            Eduardo Mangeli
%% 
%% ufrj-quickref.tex -- CoppeTeX quick reference, in English.
%%
%% GERADO de ufrj.dtx. Nao edite este arquivo: edite o modulo `quickref' do
%% ufrj.dtx e rode `pdflatex ufrj.ins'.
...
\begin{center}
  ...
  {\normalsize Class \texttt{ufrj}, \ufrjversion\ --- every option, command and
  environment}\\[6pt]
  \begin{minipage}{0.86\textwidth}\small
  This is the short companion to \texttt{ufrj.pdf}, the full manual, which is
  written in Portuguese. A UFRJ thesis may be written in Portuguese, English or
  Spanish (CEPG Resolution 302/2024, art.~57); this sheet is for whoever writes
  it in English. A work starts with the class and the style of its academic
  unit --- \verb|\documentclass[dsc]{ufrj}| and \verb|\usepackage{ufrj-coppe}|,
  say.

  \medskip
  \textbf{Everything on this sheet is the CLASS}, which names no unit: it is
  the Manual UFRJ/SiBI, and it is the same wherever the work is written. What
  belongs to a unit --- its name, its Programas or Courses, its logo, its own
  options --- is on the unit's own sheet: \texttt{coppe-quickref.pdf} for
  COPPE, \texttt{poli-quickref.pdf} for the Escola Politécnica.
...
msc      & \textbackslash documentclass[msc]\{ufrj\}     & \pre & M.Sc.\ dissertation. \\
dsc      & \textbackslash documentclass[dsc]\{ufrj\}     & \pre & D.Sc.\ thesis. \\
mscexam  & \textbackslash documentclass[mscexam]\{ufrj\} & \pre & M.Sc.\ qualifying exam. No CAPES sheet, no catalogue card, no reference above the abstract. \\
dscexam  & \textbackslash documentclass[dscexam]\{ufrj\} & \pre & D.Sc.\ qualifying exam. Same. \\
mscsem   & \textbackslash documentclass[mscsem]\{ufrj\}  & \pre & M.Sc.\ seminar, a requirement of some Programas and \emph{not} a qualifying exam. Same three omissions: it is not deposited. \\
grad     & \textbackslash documentclass[grad]\{ufrj\}    & \pre & Undergraduate work (TCC; ``Projeto de Graduação'' with \texttt{ufrj-poli}), presented to a Course: the title conferred is the course's, and the additional sheet carries only the catalog card, without the CAPES fields. \\
\midrule
...
spanish   & [spanish,dsc]   & \pre & Spanish. Needs \texttt{ufrj-lang-spanish.def} and \texttt{spanish-ufrj.lbx}. Three abstracts. \\
...
\multicolumn{4}{@{}l@{}}{\normalfont\bfseries\small CLASS OPTIONS --- the
  rest}\\[2pt]
numbers   & [dsc,numbers]   & \pre & Numeric citations \texttt{(1)} instead of author-date (4.1.1.1.1: parentheses or superscript, \texttt{\textbackslash supercite}); list in citation order. Not with footnotes: the class warns. \\
sempdfa   & [dsc,sempdfa]   & \pre & TURNS OFF the PDF/A-2b output, which is ON by default since v5.0: 2.2(d) requires the deposited file to be PDF/A (ISO 19005), self-contained --- embedded fonts, sRGB colour, XMP metadata, nothing external. Use it for a draft run, or for a package PDF/A rejects; the deposited file has to be PDF/A. The v4.1 option \texttt{pdfa} is still accepted and does nothing. Check with veraPDF. \\
...
semlinks  & [dsc,semlinks]  & \pre & Accepted; confirms the default (links with no colour and no frame, still clickable). Wins over the two below. \\
linkscommoldura & [dsc,linkscommoldura] & \pre & Framed links, on screen only (the frame is not printed). \\
setavermelha & [dsc,setavermelha] & \pre & Red curved arrow at the start of a wrapped listing line. Black by default. \\
linkscoloridos & [dsc,linkscoloridos] & \pre & Coloured link text. Against 2.1(b) of the UFRJ manual (black text): for on-screen reading copies, not for deposit. \\
...
semorientadornabanca & [dsc,semorientadornabanca] & \pre & Approval sheet with the examiners only. By default the advisors head the board, as 3.1.2.1.3(e) requires. \\
orientadorexamina & [dsc,orientadorexamina] & \pre & Puts the advisors and coadvisors at the head of the board on the approval sheet. Off by default: the board is what examiner declares, in order, and the advisor, who chairs it, is the first examiner (3.1.2.1.3e). \\
...
listasnosumario & [dsc,listasnosumario] & \pre & Put the pre-textual lists in the table of contents. Against NBR 6027 and the UFRJ manual (3.1.2.1.6): use only if your Programa requires it. \\
...
semmakeindex & [dsc,semmakeindex] & \pre & Do not run makeindex from the class. By default the class runs it at the end of each compilation (restricted shell escape is enough), so the lists and the index appear in any editor. \\
...
\textbackslash newufrjfloat & \textbackslash newufrjfloat\{map\}\{Map\}\{List of Maps\} & \pre & Declare your own float: caption on top, \texttt{\textbackslash source} below, own list. \\
...
\textbackslash useufrjlanguage & \textbackslash useufrjlanguage\{french\} & \pre & Load one more language pack, for quotations in a third language. \\
...
\textbackslash department & \textbackslash department\{SIGLA\} & \doc & The acronym of the Programa (or, with \texttt{grad}, of the Course), as the unit style declares it. The acronyms are on the unit's sheet; one the unit did not declare is a class error. \\
\textbackslash date & \textbackslash date\{09\}\{2026\} & \doc & Month and year of deposit, as numbers. Not the defence date. \\
\textbackslash city & \textbackslash city\{Macaé\} & \doc & The city of the work, which heads the cover and the reference above each abstract. UFRJ has campuses in Macaé and in Duque de Caxias; the unit style declares its own default, and what the author writes wins. \\
\textbackslash state & \textbackslash state\{RJ\} & \doc & Its state. \\
\textbackslash country & \textbackslash country\{Brasil\} & \doc & Its country, printed only when the language pack asks for it. \\
\textbackslash place & \textbackslash place\{Macaé\}\{RJ\}\{Brasil\} & \doc & The three at once. \\
\textbackslash university & \textbackslash university\{Universidade Federal do Rio de Janeiro\} & \doc & The university's name, for the rare work that is not UFRJ's. \\
...
\textbackslash volumefiles & \textbackslash volumefiles\{v1,v2\} & \pre & The .tex names of every volume, in order, in the preamble of all of them: the sheet count runs from the first volume to the last (2.7) and every volume carries the complete sumario (3.1.2.1.6). Compile them alternately, twice. Alias: \texttt{\textbackslash arquivosdosvolumes}. \\
\textbackslash volume & \textbackslash volume\{1\} & \doc & Which volume this is. \\
\midrule

\multicolumn{4}{@{}l@{}}{\normalfont\bfseries\small TITLE PAGE AND CAPES
  SHEET}\\[2pt]
\textbackslash areaconcentracao & \textbackslash areaconcentracao\{Engenharia de Sistemas\} & \doc & Area of concentration. On the folha adicional; on the identity sheets only under the \texttt{areanafolhaderosto} option. \\
...
\textbackslash epigrafe & \textbackslash epigrafe\{quotation\}\{author\} & \doc & Epigraph (3.1.2.2.3), on a sheet of its own, after the acknowledgements. \\
...
\textbackslash configuraresumos & \textbackslash configuraresumos\{true\}\{true\}\{true\}\{true\} & \pre & Turns the four optional elements of every abstract sheet on or off, in every language: page identification, title and author, advising, reference. By default only the reference, as in Annexes E and F; only the abstract text itself cannot be turned off. \\
...
\textbackslash ufrjfinalpage & \textbackslash ufrjfinalpage & \doc & Optional colophon, every item measured at compile time: class version, engine and its version, TeX system, \LaTeX{} format, font family and encoding, biblatex version, date, hour, operating system and machine. \\
\midrule

\multicolumn{4}{@{}p{\linewidth}@{}}{\normalfont\small Only the table of contents is mandatory (3.1.2.1.6). Every list is optional (3.1.2.2), and a unit's norm may ask for more --- see the unit's sheet.}\\[2pt]
\multicolumn{4}{@{}l@{}}{\normalfont\bfseries\small PRE-TEXTUAL LISTS ---
  illustrations, tables, abbreviations, symbols; contents last (3.1.2)}\\[2pt]
...
\textbackslash listoftables & \textbackslash listoftables & \doc & List of tables, after every list of illustrations (3.1.2). \\
...
\textbackslash makeglossarylist & \textbackslash makeglossarylist & \pre & The automatic glossary (3.1.4.2). Without it, write \texttt{theglossary} by hand. \\
\textbackslash glossaryterm & \textbackslash glossaryterm[key]\{term\}\{definition\} & \doc & Registers a glossary term where it appears; prints nothing. Alphabetical, accents included. \\
\textbackslash printglossarylist & \textbackslash printglossarylist & \doc & The glossary, right after \texttt{\textbackslash printbibliography}. \\
\textbackslash index & \textbackslash index\{term!sub\textbar see\{other\}\} & \doc & Index entry (with \texttt{makeidx}). Accented entries get an unaccented sort key; \texttt{key@term} wins. \\
...
\textbackslash illustrationwidth & \textbackslash illustrationwidth\{4cm\} & \doc & Inside a float: sets the caption and the source line to the width of the illustration (2.10). Alias: \texttt{\textbackslash largurailustracao}. \\
...
\vfill
\begin{center}\small
Full manual: \texttt{ufrj.pdf}, in Portuguese.\quad
Unit sheets: \texttt{coppe-quickref.pdf}, \texttt{poli-quickref.pdf}.\quad
Worked examples: \texttt{coppe-min-exemplo.tex} (only what is mandatory) and \texttt{coppe-max-exemplo.tex} (everything); \texttt{poli-min-exemplo.tex} and \texttt{poli-max-exemplo.tex} for an undergraduate project.\quad
