...
%%
%% ufrj-coppe.dtx  (with options: `examplept')
%% 
...
\begin{document}
  ...
  \subtitle{um exemplo de uso do CoppeTeX em português}
  \foreigntitle{Title of the Work}
  \foreignsubtitle{an example of use of CoppeTeX in Portuguese}
  ...
  % Ordem dos pré-textuais (Manual 3.1.2), que a classe confere: \maketitle,
  % \frontmatter, o resumo em português, o resumo em língua estrangeira, as
  % listas e \tableofcontents, sempre o último.
  \maketitle
  \frontmatter
  \begin{abstract}
    Este é o resumo principal em português, idioma escolhido por padrão
    quando nenhuma opção de idioma é passada para a classe \texttt{ufrj}.
  ...
  \emph{b} garante a reprodução visual fiel. Não é preciso pedir: a classe
  carrega o \texttt{pdfx}, monta os metadados a partir do título, do autor e
  das palavras-chave, e embute o perfil de cor em todo documento. A opção
  \texttt{sempdfa} desliga, para um rascunho ou para um pacote que o PDF/A não
  aceite; se você desligar, lembre que um PDF ou uma imagem incluídos fora da
  norma aparecem no dia em que entram, e não na véspera do depósito. Para conferir, use o validador veraPDF, perfil PDF/A-2b.
