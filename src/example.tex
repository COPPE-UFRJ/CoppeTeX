...
\makelosymbols
\makeloabbreviations
\usepackage{indentfirst} % se quiser indentar o primeiro paragrafo das seções
\renewcommand{\familydefault}{\rmdefault} % para usar fontes COM serifa no default

\begin{document}
  \title{Título da Tese}
  \foreigntitle{Thesis Title}
  \author{Nome do Autor}{Sobrenome}
  \advisor[UFRJ]{Prof.}{Nome do Primeiro Orientador}{Sobrenome}{D.Sc.}
  \advisor[UFRJ]{Prof.}{Nome do Segundo Orientador}{Sobrenome}{Ph.D.}
  ...
  \examiner[UFRJ]{Prof.}{Nome do Primeiro Examinador Sobrenome}{D.Sc.}
  \examiner[UFRJ]{Prof.}{Nome do Segundo Examinador Sobrenome}{Ph.D.}
  \examiner[UFF]{Prof.}{Nome do Terceiro Examinador Sobrenome}{D.Sc.}
  \examiner[UNICAMP]{Prof.}{Nome do Quarto Examinador Sobrenome}{Ph.D.}
  \examiner[USP]{Prof.}{Nome do Quinto Examinador Sobrenome}{Ph.D.}
  \department{PESC}
  \date{06}{2026}
  ...
  \areaconcentracao{Engenharia de Sistemas e Computa\c c\~ao}
  ...
  Em 2026, Geraldo Xexéo, com forte ajuda das ferramentas Claude 4.7 Opus e Max e 4.8 Opus Max, fez modificações de alta monta para suportar o novo \citetitle{manualbib}, que implementava normas recentes da ABNT.
...
Referencie a figura inteira com \verb|\autoref{fig:subs}| e cada parte com \verb|\autoref{fig:sub-a}| (\autoref{fig:sub-a}).
...
O \verb|tikz| (disponível também via \verb|tcolorbox|) desenha gráficos vetoriais no próprio \LaTeX. Um exemplo simples:
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}
    ...
    \draw[->, thick] (a) -- (b) node[midway, above] {processo};
  \end{tikzpicture}
...
Para gráficos de dados (curvas, barras), veja o \verb|pgfplots|, construído sobre o \verb|tikz|.
...
Há estilos prontos também para \verb|java|, \verb|xml|, \verb|html| e \verb|prolog|, comandos \verb|\pythoninline|/\verb|\javainline| para código no meio do texto, e o ambiente \verb|Verbatim| (com as opções acima) para a saída de programas. Para algoritmos, use o ambiente \verb|algorithm| com as palavras-chave em português \verb|\Para| e \verb|\Enquanto|, como no \autoref{alg:soma}.
...
\end{algorithm}
  ...
  A classe carrega \texttt{amsmath} em qualquer motor. Sob \hologo{pdfLaTeX} ela também carrega \texttt{amssymb} (símbolos clássicos $\mathbb{R}$, $\hbar$, $\varnothing$, \ldots). Sob \hologo{LuaLaTeX}/\hologo{XeLaTeX}, \textbf{a classe NÃO carrega \texttt{amssymb}} (ele é incompatível com o \texttt{unicode-math}); se você terminar o documento sem ter carregado nem \texttt{unicode-math} nem \texttt{amssymb}, a classe emite um \emph{ClassWarning} no fim da compilação pedindo que você adicione \verb|\usepackage{unicode-math}| ao preâmbulo (veja a~\autoref{sec:unicode-math}). Fórmulas no texto vão entre \verb|$...$|; fórmulas destacadas e numeradas usam o ambiente \verb|equation|.
  ...
  Ao migrar para \hologo{LuaLaTeX} (ou \hologo{XeLaTeX}), \textbf{não carregue} \texttt{amssymb}: ele é o pacote AMS legado, escrito para as fontes Type~1 de 8~bits (\texttt{msam}/\texttt{msbm}), e colide com \texttt{unicode-math} -- o pacote moderno que controla a fonte matemática OpenType. Por isso a classe \texttt{coppe} já \emph{omite} \texttt{amssymb} sob motores Unicode e emite um aviso na compilação (procure por \texttt{Class coppe Warning} no log) lembrando que cabe a você completar o preâmbulo:
  \begin{verbatim}
  ...
  A classe carrega o \verb|hyperref|: o sumário, as citações e os \verb|\ref|/\verb|\autoref| viram \emph{links} clicáveis e o leitor de PDF ganha um painel de \emph{bookmarks} (navegação por capítulos). Não é preciso configurar nada; para mudar a cor dos links, use \verb|\hypersetup{...}| no preâmbulo.

  \section{URLs}

  Para endereços, use \verb|\url{https://exemplo.org}|: o \verb|hyperref| cria o link e quebra a URL no fim da linha. Para um texto-âncora, use \verb|\href{https://exemplo.org}{texto clicável}|. Evite digitar URLs como texto comum --- elas não quebram e estouram a margem.
