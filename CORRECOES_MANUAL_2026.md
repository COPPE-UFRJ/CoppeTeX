...
- **Issues:** guarda-chuva **#112**; uma por problema, **#113 a #151**, e as
  achadas no caminho: **#152** (LuaLaTeX, ao corrigir #151; teste `rt70`) e
  **#153** (a documentação atribuía ao Manual a folha única do resumo, ao
  corrigir #114; sem teste, é texto).
- **Testes:** `tests/regressivo/rt33` a `rt69` e `rt89` — **todos falham hoje**
...
1. **Fonte única.** Classe, estilos BibLaTeX, pacotes de idioma, `max-exemplo.tex`,
   `min-exemplo.tex`, `example_*.tex`, `exemplo.bib`, `manual.bib` e `ufrj.bib` saem
   do `src/ufrj.dtx` (`ufrj.ins`). **Nunca** editar os gerados. Ficam fora do
...
3a. **`#` no texto de documentação do `.dtx` é `\#`.** Dentro de `macrocode` sai
   ...
   (`pdflatex ufrj.dtx` num rascunho) **antes** do commit, em chamada separada.
...
```powershell
python tests/regressivo/run-regressivo.py rt38          # um
python tests/regressivo/run-regressivo.py rt33 rt34 rt35  # alguns
python tests/regressivo/rt38-referencias-alinhadas-a-esquerda.py  # direto, com a saída completa
```
...
Copy-Item src\ufrj.cls,src\*.bbx,src\*.cbx,src\*.dbx,src\*.lbx,src\ufrj.ist,src\*.def _scratch\proto\
...
$env:COPPE_SRC = "D:\GitHub\CoppeTeX\_scratch\proto"; python tests\regressivo\rt38-...py
$env:COPPE_SRC = ""
```
...
### Onde fica cada coisa no `ufrj.dtx` (linhas em `b8103c7`; procurar pelo texto, que as linhas andam)

| O que | Procurar | Linha | Módulo |
|---|---|---|---|
| título de capítulo | `\titleformat{\chapter}[hang]` | 5229 | class |
| `\appendix` e títulos de apêndice | `\renewcommand\appendix`, `\newcommand\ufrj@appendixheadings` | 5248, 5241 | class |
...
| referência do resumo | `\newcommand\ufrj@refresumo` | 7805 | class |
| cabeça das folhas de resumo | `\newcommand\ufrj@resumocabeca` | 7893 | class |
...
| caixa alta do sobrenome | `\protected\def\ufrj@ucfamily` | 9167 | bbx |
...
| parte de monografia | `\newbibmacro*{ufrj:partdriver}` | 9320 | bbx |
| curso e tese | `\DeclareFieldFormat{course}`, `\DeclareBibliographyDriver{thesis}` | 9363, 9369 | bbx |
| mídia | `\newbibmacro*{ufrj:media}`, `\newbibmacro*{ufrj:mediadriver}` | 9392, 9401 | bbx |
...
| estilo numérico | `\RequireBibliographyStyle{ufrj}`, `\RequireCitationStyle{numeric-comp}` | 10147, 10183 | numbbx, numcbx |
...
| **3. Sumário e listas** | #115 coluna dos pós-textuais · #116 nome e traço nas listas · #148 ordem das listas | Mesmo envoltório `\ufrj@wrapstarttoc`; #116 usa o traço de #150 |
...
- **Armadilha:** nada de elástico (`\baselineskip plus .2\baselineskip`). O
  ...
  do título menor que uma entrelinha. E o `rt42` falhou só pela medida, porque o
  `Documento.ok` do `medidas.py` olhava só se o PDF existia — agora exige também
  nenhuma linha `!` no `.log`.

### #115 — pós-textuais na coluna do sumário · `rt35` · FEITA
- `\protect\numberline{}` antes do título nas entradas: `\@chapter` (ramos de
  ...
  `\ufrj@listtoc`.
- O `\ufrj@tocnumberline` mede a largura do número; com `{}` vazio não altera a
  coluna. Os marcadores do PDF não ganham espaço na frente.
- Provado: `rt35`; `r04`, `r05`, `r07`, `r08`, `r95`, `r97`, `r98`.

### #116 — nome específico e traço nas listas · `rt36` · FEITA (com #150)
- No `\ufrj@wrapstarttoc`: se a extensão não é `toc` e existe
  `\ufrj@listname@<ext>`, `\let\numberline\ufrj@listnumberline`, que escreve
  `\ufrj@listname\nobreakspace#1\nobreakspace<traço>\space`.
- Tabela de nomes: `lof`→`\figurename`, `lot`→`\tablename`, `loq`→`\quadroname`,
  `lol`→`\lstlistingname`, `loa`→`\algorithmcfname`; `\newufrjfloat{nome}{Nome}{…}`
  define `\ufrj@listname@lonome` = `Nome`.
- `\l@figure`, `\l@table`, `\l@quadro`, `\l@lstlisting`, `\l@algocf` e o
  `\l@<float>` do `\newufrjfloat` → `\@dottedtocline{1}{0pt}{0pt}` (no
  `\AtBeginDocument`, porque `listings` e `algorithm2e` definem os seus depois).
- O traço é o de #150: travessão.
- Provado: `rt36`, `rt69`; `r01`, `r08`, `r09`, `r18`.

### #150 — travessão · `rt69` · FEITA (decisão: travessão)
...
- `\DeclareCaptionLabelSeparator{emdash}` e `labelsep=emdash`;
  ...
  `\ufrj@appendixheadings`; `\ufrj@tocapp`/`\ufrj@tocanx`; a lista (#116).
- A referência do resumo (`--` no `\ufrj@refresumo`) fica para a #119.

### #148 — ordem das listas · `rt67` · FEITA
...
- Provado: `rt67`; `r26` (gerador compila); `conferir-manual` limpo.

### #114 — título RESUMO/ABSTRACT · `rt34` · FEITA
- `\ufrj@res@heading{<idioma>}` no começo de `\ufrj@resumocabeca`: centralizado,
  negrito, caixa alta, 4 mm de respiro. Não fica sob `\configuraresumos`.
- Provado: `rt34`; `r15`, `r16`, `r23`, `r25`, `r27`, `r28`; folha conferida.
...
### #119 — referência do resumo · `rt39` · validada
- `\ufrj@refresumo`: `\raggedright`; `\textbf{\ufrj@selecttitle}` e o subtítulo
...
### #123 — dedicatória e epígrafe · `rt43` · validada
...
### #117 — notas de rodapé · `rt37` · validada
```latex
...
### #120 — listagens dentro da margem · `rt40` · validada
...
### #121 — subalíneas · `rt41` · validada
```latex
\newlength\ufrj@hifen
\settowidth\ufrj@hifen{-}
\setlist[alineas,2]{label=-,labelindent=0pt,labelwidth=\ufrj@hifen,
...
- **Armadilha já vista:** `labelwidth=*` com `labelsep=0.5em` põe o hífen no lugar
  ...
  passava no teste por isso, e o `rt41` ganhou a cobrança do vão (até 7 pt).
...
### #118 — lista de referências à esquerda · `rt38` · validada
- `\AtBeginBibliography{\raggedright \hyphenpenalty=\@M \exhyphenpenalty=\@M}` no
  `ufrj.bbx` (o biblatex lê o `.bbx` com `@` como letra; no protótipo funcionou).
- Conferir URLs longas (quebram pelo `url`), e as folhas 57–61 do max-exemplo.

### #131 — subtítulo · `rt50` · validada para `title`
```latex
...
### #134 — parte de monografia · `rt53` · proposta
- `ufrj:partdriver`: depois de `in:`, `bookauthor` (`\printnames{bookauthor}` com
  o formato de autor) **ou** `ufrj:bookeditor`; título do livro pela macro de
...
### #132 — expressões latinas · `rt51` · validada
...
### #143 — mídia · `rt62` · proposta
- `ufrj.dbx`: `director` e `producer` como `type=list,datatype=name`.
- `ufrj:media`: `\printnames` em ordem direta, sem caixa alta, com "Direção:" e
  "Produção:" (strings nos `.lbx`).
- `ufrj:mediadriver`: `version` antes da imprenta (lugar da edição).
- Conferir filme-example e videogame-example do max-exemplo.

### #141 — ordenação sem artigo · `rt60` · validada
```latex
...
### #142 — periódico corrente · `rt61` · validada
...
### #145 — hífen no intervalo · `rt64` · validada
...
### #146 — mês no idioma da publicação · `rt65` · proposta
- Em `\ufrj@abntdate`, trocar `\mkbibmonth` por uma macro que olha
...
### #138 — chamada pela entrada de título · `rt57` · proposta
...
### #139 — mesmo sobrenome e ano · `rt58` · validada
```latex
...
```
- No `ufrj.cbx` (o `authoryear-comp` liga `uniquename` depois do `.bbx`).

### #140 — sistema numérico · `rt59` · validada
- `ufrj-numeric.cbx`: `\DeclareCiteCommand{\cite}[\mkbibparens]{…}` e
  `\DeclareCiteCommand{\parencite}[\mkbibparens]{…}` copiando o `numeric-comp`
  (Apêndice A).
- `ufrj-numeric.bbx`: `\DeclareFieldFormat{labelnumberwidth}{#1}` (e
  `shorthandwidth`).
...
### #144 — chamada de entidade hierárquica · `rt63` · proposta
...
- **Não** mudar a regra da lista (comentário junto de `\ufrj@ucfamily`: nome com
  ...
  - documentar isso para o aluno no manual (`ufrj.dtx`, seção de referências;
    `src/manual.tex`; quickref), com exemplo de `.bib`;
  - o `exemplo.bib` tem de dar o exemplo certo (#149: `{Brasil. Supremo Tribunal
    Federal}` → `{BRASIL. Supremo Tribunal Federal}`; o `rt68` cobra);
  - a chamada é outra história (esta issue): com o `.bib` digitado em caixa alta,
    ...
    implementar; o `rt63` cobra o resultado, não o caminho.

### #147 — m-diss · `rt66` · FEITA
...
### #149 — conteúdo dos exemplos · `rt68` · proposta
...
### #151 — a prova roda os verificadores · `rt89` · FEITA
...
- Continua aberta a discussão: estender `conferir-norma.py` com as medidas de
  rt33–rt43, ou confiar na suíte de regressão.

### #152 — LuaLaTeX com `fontenc` T1 · `rt70` · FEITA
...
- Provado: `rt70`, `r14`, `r30` pelo nome; sonda com `\emph`, negrito itálico,
...
### #124 — `pdfa` por padrão · `rt44` · proposta (decisão na fase 0)
- `\@ufrjpdfatrue` por padrão, `\DeclareOption{sempdfa}{\@ufrjpdfafalse}`, manter
...
### #125 — `\illustrationwidth` · `rt45` · proposta
- `\newcommand\illustrationwidth[1]{\captionsetup{width=#1}\def\ufrj@ilwidth{#1}}`
  e `\let\largurailustracao\illustrationwidth`; `\cpsource` compõe num
  `\parbox{\ufrj@ilwidth}` centralizado quando definido (local ao flutuante).
- `conferir-manual.py` cobra documentação do comando novo.

### #126 — `\volumefiles` · `rt46` · proposta
...
### #127 — letras dobradas · `rt47` · proposta
- Contador `\ufrj@Alph`: 1–26 → A–Z; 27 → AA, 28 → AB… (e `\ufrj@alph`).
...
- #150 já saiu (travessão, ver acima). Para as outras, a recomendação da issue
  vale como decisão se não houver resposta; registrar no commit. `rt48`, `rt49`
  são neutros quanto à decisão. `r19` muda se #128 for "tudo em português".
- #130 não tem teste: se sair o logotipo da folha de rosto, criar `r71` (o `rt70`
  ficou com o #152). Os
...
Acrescentado ao fim das cópias geradas (`ufrj.cls`, `ufrj.bbx`, `ufrj.cbx`,
`ufrj-numeric.cbx`, `ufrj-numeric.bbx`) numa pasta de rascunho. **Não copiar
assim para o `.dtx`**: cada trecho vai para o lugar da definição que substitui.
Com ele, passaram rt33–rt43, rt50, rt51, rt58–rt61 e rt64; os demais testes continuaram
falhando, como deviam.

### A.1 `ufrj.cls`

```latex
%% rt33: titulos em corpo 12
\titleformat{\chapter}[hang]
  {\normalfont\normalsize\bfseries}
  {\if@ufrjappendix\MakeUppercase{\appendixname}\ \thechapter\ \textendash\else\thechapter\fi}
  {1ex}{\MakeUppercase}
\titleformat{name=\chapter,numberless}[hang]
  {\normalfont\normalsize\bfseries\filcenter}{}{0pt}{\MakeUppercase}
\titleformat{\section}
  {\normalfont\normalsize}{\thesection}{1ex}{\MakeUppercase}
\renewcommand\ufrj@appendixheadings{%
...
%% rt42: uma linha em branco antes e depois dos titulos
\titlespacing*{\section}{0pt}{\baselineskip}{\baselineskip}
\titlespacing*{\subsection}{0pt}{\baselineskip}{\baselineskip}
\titlespacing*{\subsubsection}{0pt}{\baselineskip}{\baselineskip}
\titlespacing*{\paragraph}{0pt}{\baselineskip}{\baselineskip}

%% rt34: titulo centralizado nas folhas de resumo
\pretocmd{\ufrj@resumocabeca}{%
  \begin{center}\bfseries
    \MakeUppercase{\@nameuse{ufrj@str@\ufrj@reslang @abstractname}}%
  \end{center}\par\vspace*{6mm}}{}{}

%% rt35: pos-textuais na coluna dos titulos do sumario
\def\@chapter[#1]#2{%
  \ifnum \c@secnumdepth >\m@ne
    \if@mainmatter
      ...
      \if@ufrjappendix
        \if@ufrjannex
          \addcontentsline{toc}{chapter}{\protect\numberline{}\protect\ufrj@tocanx{\thechapter}#1}%
        \else
          \addcontentsline{toc}{chapter}{\protect\numberline{}\protect\ufrj@tocapp{\thechapter}#1}%
        \fi
      \else
...
\defbibheading{bibliography}{%
  \chapter*{\ufrjstring{references}}%
  \addcontentsline{toc}{chapter}{\protect\numberline{}\ufrjstring{references}}}
...
%% rt36: nome especifico e traco nas listas de ilustracoes e de tabelas
\newcommand\ufrj@listnumberline[1]{\ufrj@listname\nobreakspace#1\nobreakspace\textendash\space}
\def\ufrj@listname@lof{\figurename}
\def\ufrj@listname@lot{\tablename}
\def\ufrj@listname@loq{\quadroname}
\def\ufrj@listname@lol{\lstlistingname}
\def\ufrj@listname@loa{\algorithmcfname}
\renewcommand\ufrj@wrapstarttoc{%
  \let\ufrj@orig@starttoc\@starttoc
  \def\@starttoc##1{%
    \ufrj@checkwrites{##1}%
    \begingroup
      \def\ufrj@@ext{##1}\def\ufrj@@toc{toc}%
      \ifx\ufrj@@ext\ufrj@@toc \let\numberline\ufrj@tocnumberline
      \else
        \@ifundefined{ufrj@listname@##1}{}{%
          \expandafter\let\expandafter\ufrj@listname\csname ufrj@listname@##1\endcsname
          \let\numberline\ufrj@listnumberline}%
      \fi
      \ufrj@orig@starttoc{##1}%
    \endgroup}}
\AtBeginDocument{%
...
\let\ufrj@orig@newufrjfloat\newufrjfloat
\renewcommand{\newufrjfloat}[3]{%
  \ufrj@orig@newufrjfloat{#1}{#2}{#3}%
  \expandafter\def\csname ufrj@listname@lo#1\endcsname{#2}%
  \AtBeginDocument{\expandafter\let\csname l@#1\endcsname\l@figure}}

%% rt37: nota de rodape alinhada e sem quebrar entre folhas
\renewcommand\@makefntext[1]{%
  \setbox\@tempboxa\hbox{\@makefnmark}%
  \noindent\hangindent\wd\@tempboxa\hangafter\@ne
  \box\@tempboxa #1}
...
%% rt39: referencia do resumo
\renewcommand\ufrj@refresumo{%
  \if@ufrjrefresumo
    \if@ufrjexame\else
      {\singlespacing\noindent\raggedright
        \nohyphens{%
          \MakeUppercase{\@authsurn}, \@authname.
          \textbf{\ufrj@selecttitle}%
          \@ifundefined{ufrj@subtitle@\ufrj@mainlang}{}{%
            : \ufrj@selectsubtitle}.
          \local@cityname, \ufrj@docyear.
          \local@doctype\ (\@degreename\ em \local@deptname)\ \textemdash\
          \local@instname, \local@universityname,
          \local@cityname, \ufrj@docyear.}\par}%
      \vspace*{4mm}
    \fi
  \fi}

%% rt40: numero de linha das listagens dentro da mancha
\lstset{xleftmargin=2em}
\patchcmd{\pythonstyle}{numbersep=0.5em,}{numbersep=0.5em,xleftmargin=2em,}{}{}

%% rt41: subalinea com hifen, sob a primeira letra do texto da alinea, e um espaco
\newlength\ufrj@hifen
\settowidth\ufrj@hifen{-}
\setlist[alineas,2]{label=-,labelindent=0pt,labelwidth=\ufrj@hifen,
  labelsep=0.333em,leftmargin=!,align=left,itemsep=0pt}

%% rt43: dedicatoria e epigrafe do meio da mancha
...
%% rt59: aviso de nota de rodape com o sistema numerico
\if@ufrjnumeric
  \AtBeginDocument{%
    \let\ufrj@orig@footnote\footnote
    \renewcommand\footnote{%
      \ClassWarningNoLine{ufrj}{Sistema numerico com nota de rodape: a
...
### A.4 `ufrj-numeric.cbx` e `ufrj-numeric.bbx`
