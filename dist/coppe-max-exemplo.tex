%%
%% This is file `coppe-max-exemplo.tex',
...
\documentclass[dsc,setavermelha]{ufrj}
\usepackage{ufrj-coppe}
%% coppe-max-exemplo.tex -- tudo o que a classe oferece e o Manual prevê, inclusive o
%% opcional. Para só o obrigatório, veja o coppe-min-exemplo.tex.
%%
...
%% orientadorexamina, coorientador, resumosemreferencia, rascunhoficha,
%% simbolosalfabeticos, semmorewrites. O manual da classe descreve cada uma.
%%
%% Opções de classe que valem conhecer aqui:
%%  - sempdfa: DESLIGA o PDF/A-2b, que a classe produz por padrão porque é o
%%    formato que o depósito exige (Manual 2.2d). Este exemplo não a usa, e por
%%    isso sai em PDF/A; a razão está na seção "O arquivo de depósito: PDF/A",
%%    no capítulo de regras da COPPE.
...
\begin{document}
  ...
  % Coorientador: a COPPE nao tem -- so orientadores, um ou mais --, e por
  % isso este exemplo nao usa \coadvisor. A classe o oferece, com os mesmos
  % quatro argumentos de \advisor, para as unidades que tem coorientador.
  ...
  % A banca da folha de aprovacao e o que se declara aqui, na ordem: nome,
  % titulacao e instituicao de cada membro (3.1.2.1.3e). O orientador, que
  % preside a banca, e o primeiro membro. A instituicao e obrigatoria, mas pode
  % ficar vazia: \examiner{Nome}{D.Sc.}{} compila sem erro.
  \examiner{Nome do Primeiro membro da banca Sobrenome}{D.Sc.}{UFRJ}
  \examiner{Nome do Segundo membro da banca Sobrenome}{Ph.D.}{UFRJ}
  \examiner{Nome do Terceiro membro da banca Sobrenome}{D.Sc.}{UFF}
  \examiner{Nome do Quarto membro da banca Sobrenome}{D.Sc.}{UNICAMP}
  \examiner{Nome do Quinto membro da banca Sobrenome}{D.Sc.}{PUC-Rio}
  \department{PESC}
  \date{06}{2026}
  % Data da defesa, na folha de aprovacao (3.1.2.1.3d). Sem esta linha a folha
  % sai com "a ser determinada", que e o que se quer enquanto a data nao
  % existe; o r10 do regressivo cobra esse caso.
  \approvaldate{1 de janeiro de 2026}
  %
  ...
  \projectname{Nome do Projeto de Pesquisa ao qual o trabalho se vincula (caso haja)}
  \fundingagency{Conselho Nacional de Desenvolvimento Científico e
                  Tecnológico}{CNPq}
  \fundingagency{Fundação Carlos Chagas Filho de Amparo à Pesquisa do
  ...
  % ORDEM DOS ELEMENTOS PRÉ-TEXTUAIS (Manual 3.1.2). A classe confere esta
  % ordem: um elemento fora dela é erro de compilação, e a mensagem diz o que
  % mover. Os obrigatórios estão em MAIÚSCULAS; os outros são opcionais.
  %   \maketitle ......... CAPA, FOLHA DE ROSTO e FOLHA ADICIONAL
  %   errata ............. se houver, antes da folha de aprovação
  %   \frontmatter ....... FOLHA DE APROVAÇÃO
  %   \dedication ........ dedicatória
  %   agradecimentos ..... \chapter*{Agradecimentos}
  %   \epigraph .......... epígrafe
  %   resumos ............ RESUMO EM PORTUGUÊS, sempre o primeiro, e depois
  %                        o RESUMO EM LÍNGUA ESTRANGEIRA
  %   listas ............. de ilustrações, de tabelas, de abreviaturas e
  %                        siglas e de símbolos, nessa ordem
  %   \tableofcontents ... SUMÁRIO, sempre o último
  \maketitle
  ...
  \begin{abstract}% a folha do Anexo E; \setupabstracts liga os outros 3
  ...
  %
  % Obrigatório e opcional, na ordem da 3.1.2 (ilustrações antes de tabelas):
  %  - \listoffigures .......... opcional (3.1.2.2.4, lista de ilustrações);
  %  - \listofframes, \listofprograms, \listofalgorithms: acréscimo da
  %    Norma COPPE (§10), quando há quadro, programa ou algoritmo -- que são
  %    ilustrações (2.10), e por isso vêm antes da lista de tabelas;
  ...
  %  - \tableofcontents ........ OBRIGATÓRIO (Manual 3.1.2.1.6), por último.
...
Este exemplo sai em \textbf{PDF/A-2b} sem pedir nada: desde a v5.0 é o padrão da classe.
\begin{verbatim}
\documentclass[dsc,setavermelha]{ufrj}
\usepackage{ufrj-coppe}
\end{verbatim}
Quem precisar de um PDF comum --- numa compilação de rascunho, ou com um pacote que o PDF/A não aceite --- escreve \verb|sempdfa| entre as opções. Esta seção explica o que isso é, por que a COPPE exige, o que a classe faz e o que pode dar errado.
...
\chapter{Alguns outros exemplos úteis}
...
use \verb|\href{https://exemplo.org}{texto}|. Evite digitar URLs como texto comum --- elas não quebram e estouram a margem.
  ...
  \chapter{Truques de \texorpdfstring{\LaTeX}{LaTeX}}
  ...
  \abbrev[ITU]{ITU}{União Internacional de Telecomunicações}
  ...
  \nocite{patente-example,legislacao-example,m-norma,mapa-example,partitura-example}
  \nocite{jurisprudencia-example}
  \nocite{videogame-example,filme-example,semautor-example,semautorart-example}
  \nocite{m-4236,jornal-example}
  \printbibliography
...
No modelo \CoppeTeX{} os anexos devem obrigatoriamente vir depois dos apêndices e usam o comando novo (versão 3.5 em diante) \verb|\annex|
...
%% 
%%
%% End of file `coppe-max-exemplo.tex'.
