...
do projeto CoppeTeX, versão v5.0". O que muda é a organização. A classe passa a
se chamar `ufrj` e a implementar só o Manual da UFRJ/SiBI, e tudo o que era da
COPPE — o nome do Instituto, os treze
...
| Seus pré-textuais estão fora da ordem da 3.1.2 e você usa `\documentclass{coppe}` | Até 22/09/2026 a classe `coppe` rebaixava isso a aviso; hoje é **erro**, como na classe `ufrj`. A mensagem diz o que mover e para onde: as listas vêm antes do sumário, e o sumário é o último pré-textual. |
| Você contava com o orientador na banca da folha de aprovação sem declará-lo | Declare-o como o **primeiro** `\examiner` — ele preside a banca (3.1.2.1.3e) —, ou use a opção `orientadorexamina`, que põe os orientadores à frente dos examinadores. Na 5.0 a banca é o que o trabalho declara (#165). |
